...
fn enum_http(address: &str, port: u16, service: &str, basedir: &str, deep: bool) {
...
fn enum_smtp(address: &str, port: u16, basedir: &str, deep: bool) {
    let cmd = format!(
...
fn enum_pop3(address: &str, port: u16, basedir: &str, deep: bool) {
    let cmd = format!(
...
fn enum_imap(address: &str, port: u16, basedir: &str, deep: bool) {
    let cmd = format!(
...
fn enum_ftp(address: &str, port: u16, basedir: &str, deep: bool) {
    let cmd = format!(
...
fn enum_smb(address: &str, port: u16, basedir: &str, deep: bool) {
...
fn enum_mssql(address: &str, port: u16, basedir: &str, deep: bool) {
    let cmd = format!(
...
fn enum_mysql(address: &str, port: u16, basedir: &str, deep: bool) {
    let cmd = format!(
...
fn enum_oracle(address: &str, port: u16, basedir: &str, deep: bool) {
    let cmd = format!(
...
fn enum_nfs(address: &str, port: u16, basedir: &str, deep: bool) {
    let cmd = format!(
...
fn enum_snmp(address: &str, port: u16, basedir: &str, deep: bool) {
    let cmds = vec![
        format!(
...
fn enum_dns(address: &str, port: u16, basedir: &str, deep: bool) {
...
fn enum_rdp(address: &str, port: u16, basedir: &str, deep: bool) {
    let cmd = format!(
...
fn enum_vnc(address: &str, port: u16, basedir: &str, deep: bool) {
    let cmd = format!(
...
fn enum_generic_tcp(address: &str, port: u16, basedir: &str, deep: bool) {
    let cmd = format!(
...
fn enum_generic_udp(address: &str, port: u16, basedir: &str, deep: bool) {
    let cmd = format!(
...
fn main() {
    let matches = App::new("Network Scanner")
        ...
        .arg(Arg::new("output")
            ...
            .required(true))
        .arg(Arg::new("deep")
            .about("Perform deep scan with second stage commands")
            .short('d')
            .long("deep")
            .takes_value(false))
        .get_matches();
    ...
    for service in identified_services {
        ...
        match service.2.as_str() {
            s if s.contains("http") => enum_http(&service.0, service.1.abs() as u16, &service.2, outdir, deep),
            s if s.contains("smtp") => enum_smtp(&service.0, service.1.abs() as u16, outdir, deep),
            s if s.contains("pop3") => enum_pop3(&service.0, service.1.abs() as u16, outdir, deep),
            s if s.contains("imap") => enum_imap(&service.0, service.1.abs() as u16, outdir, deep),
            s if s.contains("ftp") => enum_ftp(&service.0, service.1.abs() as u16, outdir, deep),
            s if s.contains("mssql") => enum_mssql(&service.0, service.1.abs() as u16, outdir, deep),
            s if s.contains("mysql") => enum_mysql(&service.0, service.1.abs() as u16, outdir, deep),
            s if s.contains("oracle") => enum_oracle(&service.0, service.1.abs() as u16, outdir, deep),
            s if s.contains("nfs") => enum_nfs(&service.0, service.1.abs() as u16, outdir, deep),
            s if s.contains("snmp") => enum_snmp(&service.0, service.1.abs() as u16, outdir, deep),
            s if s.contains("dns") => enum_dns(&service.0, service.1.abs() as u16, outdir, deep),
            s if s.contains("rdp") => enum_rdp(&service.0, service.1.abs() as u16, outdir, deep),
            s if s.contains("vnc") => enum_vnc(&service.0, service.1.abs() as u16, outdir, deep),
            s if s.contains("smb") => enum_smb(&service.0, service.1.abs() as u16, outdir, deep),
            _ => {
                if service.1 > 0 {
                    enum_generic_tcp(&service.0, service.1.abs() as u16, outdir, deep);
                } else {
                    enum_generic_udp(&service.0, service.1.abs() as u16, outdir, deep);
                }
            }
        }
    }
